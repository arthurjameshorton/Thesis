...
In the Bertini Cascade model the incident hadron enters the target nucleus (modeled as a set of spherical shells) and creates secondary particles based on the free space nucleon-nucleon cross sections for that collision.  
These secondaries move in straight lines about the nucleus, reflecting from or transmitting through the shells and producing secondary and tertiary collisions.  
...
The trigger used for the Z+jet analysis searches for events with two loose (see below) electrons\footnote{In this context ``electron'' refers to both electrons and positrons} with energy above 15 GeV.  
...
Jets within $\Delta R$ = 0.2 of a photon identified by a separate algorithm (see Sec.~\ref{Sec:Photons}) are removed from the jet collection, as are jets within $\Delta R$ = 0.35 of a lepton.  
...
The ``BadLoose'' criterion been found to provide a 99.5\% efficiency at selecting good jets above 20 GeV~\cite{ATLAS-CONF-2015-029}.  

In 2016 the average number of interactions per bunch crossing in the LHC in a single lumiblock (a period of time of about 1 minute) reached as high as 52.2 (a quantity known as $\mu$)  on Oct. 14th, 2016.  
With such a high luminosity the probability of misidentifying a pileup jet as being part of the hard scattering interaction of interest becomes quite high.  
...
These two cuts have also been found to be uncorrelated in the past by varying the cuts simultaneously~\cite{Aad:2011he}.  
...
An example of the response distribution in the $\gamma$+jet channel at EM scale for photons with transverse momentum between 25 and 45 GeV in the 2016 dataset is shown in Fig.~\ref{ExampleRespDist}, with additional examples being found in appendix~\ref{App:2016}.  
...
This affect doesn't have a large effect on $\gamma$+jet as it does not have a low enough reach in energy (see Fig.~\ref{Fig:GammaJetEM2016}).  
...
Bin merging is then done by beginning with the first bin and determining if $\left(data/MC\right)_{\mathrm nominal}/\left(data/MC\right)_{\mathrm varied}$ differs from 1 by more than 1.5 times the statistical uncertainty.  
...
This procedure is done starting from both the highest and lowest $p_{\mathrm T}$ bins, where in the end the direction yielding the largest number of merged bins is used.  
...
\begin{figure}[!ht]
  ...
  {\small Distribution of the 100 pseudo-experiments used to estimate the statistical uncertainty on the effect of loosening the subleading jet cut.  This particular distribution is used to determine the uncertainty at EM+GSC scale in 2016 data for $\gamma$+jet events with a photon $p_{\mathrm T}$ between 105 and 125 GeV.  }
  \label{ExamplePseudo}
\end{figure}
...
The effect of the subleading jet $p_{\mathrm T}$ cut is mostly to remove FSR.
...
The effect of this cut variation on the ratio of the measured response in data and MC is found to be quite small (less than 0.2\%) as can be seen in Fig.~\ref{Fig:J2GJetEM2016}.
...
This dependence of the measured response on the subleading jet cut is smaller when using the MPF when compared to a $p_{\mathrm{T}}$-balance based calibration, and is therefore a strength of the MPF.  



\subsubsection{Variation of the Delta Phi Cut}
The effect of the $\Delta\phi$ cut is mostly to remove ISR.  
...
\begin{figure}[!ht]
  ...
  \begin{subfigure}{.5\textwidth}  \centering
    ...
    \caption{With rebinning}
  \end{subfigure}
  \caption{Effect of varying the muon momentum scale up and down on the EM+GSC scale MPF response using Z+jet events in 2016 before and after rebinning.  Note that the y-axis scale is different from Fig.~\ref{Fig:EGScaleGJetEM2016}.}
...
\begin{figure}[!ht]
  ...
  \begin{subfigure}{.5\textwidth}  \centering
    ...
    \caption{With rebinning}
  \end{subfigure}
  \caption{Effect of varying the momentum smearing in the ID up and down on the EM+GSC scale MPF response using Z+jet events in 2016 before and after rebinning. Note that the y-axis scale is different from Fig.~\ref{Fig:EGScaleGJetEM2016}.}
...
\begin{figure}[!ht]
  ...
  \begin{subfigure}{.5\textwidth}  \centering
    ...
    \caption{With rebinning}
  \end{subfigure}
  \caption{Effect of varying the momentum smearing in the MS up and down on the EM+GSC scale MPF response using Z+jet events in 2016 before and after rebinning. Note that the y-axis scale is different from Fig.~\ref{Fig:EGScaleGJetEM2016}.}
...
This illustrates the relative immunity of the MPF technique to pileup in general.
There is a small potential effect at low $p_{\mathrm T}$ where the numerous low energy pileup jets could have a larger effect, but even in this region the agreement never changes by more than a third of a percent.  
The small pileup dependence of the MPF on pileup can be further seen in Figs.~\ref{RespVsMUJetEM2016} and~\ref{Fig:RespVsNPVGJetEM2016}, which shows the dependence of the measured response on the average number of collisions per lumiblock ($\mu$) and the number of reconstructed vertices (NPV) respectively.  
This in built pileup resiliency is another strength of the MPF method.  
...
Two cuts are chosen which ideally have a large impact on the signal acceptance while being uncorrelated.  
...
{\small Diagram showing the signal region (A) and sidebands that are used in the photon signal purity measurement.  LNT is ``Loose-Not-Tight''.  }
\label{ABCD}
\end{figure}
...
This potential ``signal leakage'' is accounted using a signal only MC sample and measuring the amount of signal in each background region relative to the amount in the signal region.  
...
Assuming that the distribution of signal events is the same in data as it is in MC
...
It has been studied in the past using background-only MC samples, where it was found that the relative response difference between signal and background could conservatively be covered by assuming a flat 5\% response difference across all $p_{\mathrm T}$~\cite{ATLAS-CONF-2012-063}.  
...
Note that by their very nature (they contain a larger fraction of EM components) the response of dijet fakes is closer to the response of photons than most jets because they have a large EM component.   
...
An additional uncertainty is included to cover any potential non-closure this calibration procedure could cause when using samples which have not been produced using Pythia.  
...
The detector simulation is still the default ATLAS setup, which uses GEANT4 with the Bertini Cascade Model transitioning to the Fritiof Model combined with the Precompound model (called FTFP) at higher energy.   
...
The uncertainty on the JES for the EM+GSC scale using Z+jet events is shown in Fig.~\ref{Fig:ZJetSystsEM2016Main}; it is on the order of 0.5\% between $\sim$ 30 GeV and 500 GeV.  
